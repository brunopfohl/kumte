...
import * as DocumentPicker from '@react-native-community/document-picker';
...
export class FileService {
  ...
  static async importDocument(type: 'pdf' | 'image'): Promise<Document | null> {
    try {
      ...
      const newDoc = {
        title: result.name || (type === 'pdf' ? 'Imported PDF' : 'Imported Image'),
        type: type,
        uri: result.uri
      };
      ...
      Alert.alert('Error', `Failed to import document: ${error.message || 'Unknown error'}`);
      return null;
    }
  }
